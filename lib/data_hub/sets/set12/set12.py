...
def load(cfg):
    ...
    # -- color or bw --
    use_bw = optional(cfg,"bw",True)
    assert use_bw,"Set12 must be black&white."
    ...
    # -- create objcs --
    data = edict()
    data.tr = SET12(iroot,sroot,"train",noise_info,nsamples.tr,isizes.tr)
    data.val = SET12(iroot,sroot,"val",noise_info,nsamples.val,isizes.val)
    data.te = SET12(iroot,sroot,"test",noise_info,nsamples.te,isizes.te)
