...
# -- local imports --
from .paths import IMAGE_PATH_UNPAIRED as IMAGE_PATH
from .paths import IMAGE_SETS_UNPAIRED as IMAGE_SETS
from .reader import read_files,read_video

class SubmilliluxUnpaired():
...
def load(cfg):
    ...
    # -- create objcs --
    data = edict()
    data.tr = SubmilliluxUnpaired(iroot,sroot,"train",noise_info,
                          nsamples.tr,nframes.tr,isizes.tr)
    data.val = SubmilliluxUnpaired(iroot,sroot,"val",noise_info,
                           nsamples.val,nframes.val,isizes.val)
    data.te = SubmilliluxUnpaired(iroot,sroot,"test",noise_info,
