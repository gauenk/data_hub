...
from data_hub.transforms import rescale_imgs
...
class DAVISAnno():
    ...
    def __getitem__(self, index):
        ...
        if not(self.split in ["test","te"]):
            annos = read_annos(vid_files)
            annos = th.from_numpy(annos)
        else:
            annos = th.FloatTensor([])
        ...
        # -- re-scale imgs --
        clean,annos,fflow,bflow = rescale_imgs(self.iscale,clean,annos,fflow,bflow)
        fflow *= self.iscale
        bflow *= self.iscale
        ...
        return {'noisy':noisy,'clean':clean,'annos':annos,'index':index_th,
...
def load(cfg):
    ...
    data.tr = DAVISAnno(iroot,sroot,tr_set,noise_info,p.tr)
    data.val = DAVISAnno(iroot,sroot,"val",noise_info,p.val)
    data.te = DAVISAnno(iroot,sroot,"test-dev",noise_info,p.te)
